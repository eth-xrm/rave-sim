...
LEXER = re_compile(
    r"[A-Z][a-z]*|[0-9]+\.?[0-9]*([eE][-+]?[0-9]+)?|[()]|<EOS>").match
...
for sym in (
        ...
        'Cr', 'Cs', 'Cu', 'Dy', 'Er', 'Es', 'Eu', 'F', 'Fe', 'Fm', 'Fr', 'Ga',
        'Gd', 'Ge', 'H', 'He', 'Hf', 'Hg', 'Ho', 'I', 'In', 'Ir', 'K', 'Kr',
        'La', 'Li', 'Lr', 'Lu', 'Md', 'Mg', 'Mn', 'Mo', 'N', 'Na', 'Nb', 'Nd',
        'Ne', 'Ni', 'No', 'Np', 'O', 'Os', 'P', 'Pa', 'Pb', 'Pd', 'Pm', 'Po',
        'Pr', 'Pt', 'Pu', 'Ra', 'Rb', 'Re', 'Rh', 'Rn', 'Ru', 'S', 'Sb', 'Sc',
        'Se', 'Si', 'Sm', 'Sn', 'Sr', 'Ta', 'Tb', 'Tc', 'Te', 'Th', 'Ti', 'Tl',
        'Tm', 'U', 'Unh', 'Unp', 'Unq', 'Uns', 'V', 'W', 'Xe', 'Y', 'Yb', 'Zn',
        'Zr'):
        ...
        while self.tok.ttype in (LPAREN, NAME):
            ...
            if self.tok.ttype == LPAREN:
                ...
                if self.tok.tvalue in ELEMENTS:
                    ...
                    self.tok.error(
                        "'" + self.tok.tvalue + "' is not an element symbol")
                self.tok.gettoken()
